...
- 目录结构采用
  ...
  ├── schemas/                # 数据验证和序列化层
  ├── settings/
  	├── config.py # 配置管理，数据库连接配置，JWT密钥和算法配置，应用程序环境变量，第三方服务配置等
...
- 要求必须使用一下的数据库迁移命令，不能够使用py自动执行迁移
...
aerich init -t backend.settings.config.TORTOISE_ORM --location ./backend/migrations
```