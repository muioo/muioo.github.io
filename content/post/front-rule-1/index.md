...
title: "【Claude Code 规则】Claude Code 前端开发规则1"

date: 2026-01-26

description: "vue + ts + pinia + uniapp 开发前端"
...
- 你是一名ui设计师，前端采用vue+ts+pinia+uniapp，后端采用 FastAPI + Tortoise ORM + MySQL现在要设计一个专家人脸匹配系统的前端，根据我的后端api文档设计我的前端页面，要求容器不能溢出屏幕，图片要能够正常显示。
...
- 角色
  ...
  * 使用**最小调色板**：默认为**蓝色、白色和中性色**-没有华丽的渐变或不匹配的色调。
  * 遵循**60-30-10比例**：约60%背景（白色/浅灰色），约30%表面（白色/中灰色），约10%强调色（蓝色/深蓝色）。
