...
- `scripts/github-trending/` 抓取 `github.com/trending` 热门仓库，生成 `content/github-trending/{date}.md`（每日一页）+ `data/github-trending/latest.json`；由 AI 日报的服务器脚本一并执行、统一提交。
...
- 本地负责编辑普通笔记并推送到 GitHub；服务器负责拉取最新笔记后，只生成并推送 AI 日报与 GitHub 趋势相关文件。
...
- GitHub 趋势抓取使用 `GH_TRENDING_BASE`、`GH_TRENDING_PERIOD`、`GH_TRENDING_FETCH_RETRIES`、`GH_TRENDING_FETCH_TIMEOUT`、`GH_TRENDING_TIMEZONE`、`GH_TRENDING_DATE`、`GH_TRENDING_MAX_ITEMS`、`GH_TRENDING_RETENTION_DAYS` 配置；`GH_TRENDING_TOKEN` 只通过服务器环境变量注入，禁止写入仓库。
...
- `scripts/run-on-server.sh`：服务器每日执行脚本（cron 调用入口），拉取最新笔记后统一生成并提交 AI 日报与 GitHub 趋势文件。
